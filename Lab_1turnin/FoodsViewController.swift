...
class FoodsViewController: UITableViewController {
    ...
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCellWithIdentifier("FoodCell", forIndexPath: indexPath) //1
        
        let food = foods[indexPath.row] as Food //2
        
        if let nameLabel = cell.viewWithTag(100) as? UILabel { //3
            nameLabel.text = food.name
        }
        if let countryLabel = cell.viewWithTag(101) as? UILabel {
            countryLabel.text = food.country
        }
        if let ratingImageView = cell.viewWithTag(102) as? UIImageView {
            ratingImageView.image = self.imageForRating(food.rating)
        }
        return cell
    }
